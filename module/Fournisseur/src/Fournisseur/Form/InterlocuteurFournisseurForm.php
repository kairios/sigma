...
/**
 * @Author: Ophelie
 * @Date:   2015-06-10 11:02:11
 * @Last Modified by:   Ophelie
 * @Last Modified time: 2015-07-21 13:42:19
 */

namespace Fournisseur\Form;

use Zend\Form\Form;
use Fournisseur\Model\InterlocuteurFournisseurModel;
use Zend\I18n\Translator\Translator;

class InterlocuteurFournisseurForm extends Form
{
	public $fields;

	public function __construct($translator,$sm,$em=null,$request=null,$interlocuteur=null)
	{
		parent::__construct('interlocuteur-form');
		$this->setAttribute('method', 'post');

		$interlocuteurModel = new InterlocuteurFournisseurModel();
		$this->fields=$interlocuteurModel->fields;
		$this->setInputFilter($interlocuteurModel->getInputFilter());
			...
			switch($data['form']['type'])
			{
				...
				case 'select':
					...
					else
					{
						...
						switch($field)
						{
							case 'ref_societe_fournisseur':
								$f=new \Fournisseur\Entity\Fournisseur;
								if($interlocuteur->getRefSocieteFournisseur())
								{
									$f=$interlocuteur->getRefSocieteFournisseur();
									$codeFournisseur=$f->getCodeFournisseur();

									$fournisseurs=$f->getFournisseursFromForms($sm,$codeFournisseur,$f->getRaisonSociale());
								...
								{
									$fournisseurs=$f->getFournisseursFromForms($sm);
									if(is_array($fournisseurs)&&count($fournisseurs)>0)
									{
										foreach($fournisseurs as $fournisseur)
										{
							...
							case 'code_fournisseur':
								$f=new \Fournisseur\Entity\Fournisseur;
								$codes=$f->getCodesFournisseur($sm);
								if(is_array($codes)&&count($codes)>0)
								{
									foreach($codes as $code)
									{
										$options[]=array(
											'value'=>$code['code_fournisseur'],
											'label'=>$code['code_fournisseur']
										);
										$options[0]['value']=0;
